...
object Updater {
    ...
    private const val REPO = "HeavenWays/JARVISV2"
    ...
    suspend fun downloadAndInstall(
        ...
        try {
            ...
            val apk = File(dir, "jarvis-update.apk")
            if (apk.exists()) apk.delete()

            val req = Request.Builder().url(apkUrl).header("User-Agent", "JarvisAI").build()
            client.newCall(req).execute().use { resp ->
